...
export LINEAR_API_KEY="${LINEAR_API_KEY_DESIGNER:-$LINEAR_API_KEY}"
...
AGENT_MODEL=$(node -e "try{const m=JSON.parse(require('fs').readFileSync('$MODELS_FILE','utf8'));console.log(m['$AGENT']||'sonnet')}catch{console.log('sonnet')}" 2>/dev/null)

echo "[$(date '+%Y-%m-%d %H:%M:%S')] === Designer Agent starting (model: $AGENT_MODEL) ===" | tee -a "$LOG_FILE"
...
/Users/lelinh/.local/bin/claude \
  ...
  --allowedTools "Bash,Read,Write,Edit,Glob,Grep,mcp__claude_ai_Linear__list_issues,mcp__claude_ai_Linear__list_comments,mcp__claude_ai_Linear__get_issue,mcp__claude_ai_Linear__list_projects,mcp__claude_ai_Linear__list_teams,mcp__claude_ai_Linear__search_issues,mcp__plugin_playwright_playwright__*" \
  --model "$AGENT_MODEL" \
